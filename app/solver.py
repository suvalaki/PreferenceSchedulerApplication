...
shifts = np.asarray([s[0] for s in db.session.query(
    db.func.distinct(Shift.id)).order_by(Shift.id)])
...
# Contraint that an employee can only be working normal or overtime but
# not both
...
#prob.solve()
#fracGap = 0.02,
prob.solve(pulp.PULP_CBC_CMD(options = ['MaxN  1000']))
...
def update_shift_allocation(allocations, allocations_nor, allocations_ovr):
    ...
    for e in employees:
        for s in shifts:
            for p in periods:
                for a in roles:
                    if admissible_index(e, s, p, a) and \
                    allocations[e, s, p, a].value() == 1:
